export const metadata = {
    title: '开发文档指南',
    description: '了解如何为 Protium 项目创建和维护开发文档',
}

export const sections = [
    { title: '文档结构', id: 'document-structure' },
    { title: '创建新文档', id: 'creating-new-docs' },
    { title: '编写指南', id: 'writing-guidelines' },
    { title: '组件使用', id: 'component-usage' },
    { title: '贡献流程', id: 'contribution-workflow' },
]

# 开发文档指南

本指南将帮助你了解如何为 Protium 项目创建、修改和维护文档。遵循这些指南可确保文档的一致性和高质量。

## 文档结构

Protium 的文档使用 MDX（Markdown + JSX）格式，并采用以下目录结构组织：
...
{/* 可选添加 */}
export const sections = [
  { title: '第一部分', id: 'part-one' },
  { title: '第二部分', id: 'part-two' },
]
```

<Note>
Sections 部分为可选添加，主要用于渲染为左侧目录树。若不手动声明，将自动发现 markdown 中的 1、2 级标题作为目录。
</Note>

3. 在相应的 `Navi.tsx` 文件中添加导航链接：
...
<Warn>
**重要提示**：标题中必须包含英文单词，因为系统会使用英文作为锚点ID。如果标题只有中文，将无法正确生成导航定位。
</Warn>
...
2. **命名约定**：
   - 文件名使用小写字母和连字符，如 `quick-start.mdx`
...
3. **内容格式**：
   ...
   - 代码块应该指定语言，如 ```tsx
   - 添加适当的注释和说明
   - 对于重要信息，使用组件如 `<Note>` 或 `<Warning>` 突出显示
...
```mdx
    ```tsx {{ title: '示例代码', lineNumbers: true }}
    function Example() {
        return <div>Example</div>
    }
...
1. 从 `main` 分支创建新分支，使用以下命名格式：
   ...
   - 文档修复：`bugfix/docs-issue-description`
